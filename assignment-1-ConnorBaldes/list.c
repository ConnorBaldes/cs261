 ...
 * the list, the function should only remove the *head* instance of that
 ...
 * only the head of those values should be removed, and the other one should
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  ...
  struct link* next_link = list->head->next;

  while(current != NULL && current->next !=NULL && x != 0) {

    if(cmp(val, next_link) == 0) {
 ...
 * head instance of a specified value within a given linked list.  For
 ...
 *
 * Return:
 *   This function should return the 0-based position of the head instance of
