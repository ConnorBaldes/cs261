...
struct dynarray* dynarray_create() {
  ...
  da->priority = malloc(DYNARRAY_INIT_CAPACITY * sizeof(int));
  assert(da->data);
  da->size = 0;
...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  int* new_priority = malloc(new_capacity * sizeof(int));
 ...
 */
void dynarray_insert(struct dynarray* da, void* val, int priority) {
  assert(da);
 ...
 *     which means that a pointer of any type can be passed.
 */
void dynarray_set(struct dynarray* da, int idx, int priority, void* val) {
  assert(da);
  assert(idx < da->size && idx >= 0);

  da->data[idx] = val;
  da->priority[idx] = priority;
}

/*
This function is being annoying
*/

int dynarray_get_priority(struct dynarray* da, int idx) {
  assert(da);

  if(idx < da->size && idx >= 0) {

    return da->priority[idx];
